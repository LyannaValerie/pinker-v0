...
Status documental corrente: **Fase 121 continua o item 10.1 do Bloco 10** com camada 2 conservadora de inteiros mais largos no backend nativo externo (`u32` + `u64` em parâmetros/locais), mantendo tese de **cobertura semântica do backend nativo** (sem prometer backend pleno).
...
- chamadas diretas com **até 3 argumentos** no recorte conservador (com cobertura consolidada de `bombom` e abertura mínima para `u32` + `u64` em params/locals nas Fases 120/121), com convenção concreta mínima `%rdi/%rsi/%rdx` e retorno em `%rax`;
...
- sem sistema global rico (apenas `eterno` literal `bombom`/`logica` em `.rodata`), sem 3+ parâmetros e sem parâmetros fora do recorte (`bombom`/`u32`/`u64`/`seta<bombom>`);
