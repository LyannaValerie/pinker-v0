...
- as seis entregas foram concluídas; a última, Issue #438 e PR #439, entregou a
  cobertura de diff somente leitura;
- a janela está `CLOSED / COMPLETE` e permanece apenas como registro histórico;
- a janela não altera nem renumera os 52 itens;
- a janela não cria nova faixa, Fase ou Eixo;
- D2 é `NEXT_FUNCTIONAL_PRIORITY` e sua implementação permanece `NOT_STARTED`;
- nenhuma entrega da janela conta como progresso funcional dos itens do bloco;
- o Eixo A permanece incompleto e volta a prosseguir por D2.
...
| 4 | Traits / interfaces mínimas | Rust, TS, C# | polimorfismo sem herança, contratos de driver — **expandido nas Fases 226–230, 232, 234 e 244**: `trato` como contrato estático, chamada por método, `impl Trato para Tipo { ... }` com receiver explícito, resolução nominal, receiver nominal `ninho` no recorte opaco, cobertura completa, múltiplos contratos, desambiguação qualificada, tipo explícito `trato<Nome>`, materialização `virar`, snapshot, vtables e despacho dinâmico com ABI nativa; fora: coerções, default methods, downcasting/upcasting, herança, objetos de múltiplos tratos, igualdade e overloading amplo |
| 5 | Error handling estruturado | C#, Python, Rust | recuperação sem abort, relatório de erros do compilador — **expandido nas Fases 223–224, 231, 237, 240 e 241**: `tentar` com braços `sucesso`/`falha`, `propagar` para retorno antecipado explícito sobre leque de resultado declarado pelo usuário, ligação nomeada do valor de sucesso para continuação, forma curta `propagar?` quando há uma única variante de falha inferível com uma carga e base genérica declarável `Resultado<T,E>` via alias concreto, com lowering nativo desde a entrega; **entregue na Fase 241** a biblioteca padrão predeclarada `Resultado<T,E>` com variantes `Ok(T)`/`Erro(E)`, disponível sem declaração manual (template sintético no parser, reusando a mesma monomorfização da Fase 240) e cedendo a qualquer declaração de `Resultado` pelo usuário; fora: integração automática com erros de runtime, conversões implícitas entre resultados, operador universal `?`, inferência genérica global e métodos utilitários (`unwrap`/`map`/`and_then`) |
| 6 | Closures / funções anônimas | Rust, TS, Python | callbacks, iteradores, handlers — **expandido nas Fases 225, 238, 239, 242 e 243**: literal `carinho (...) -> tipo { ... }` não capturante em expressão, função local tipada `nova f: carinho(...) -> tipo = carinho(...) -> tipo { ... };` chamável por nome local e passagem estática dessa função como parâmetro por especialização direta (preservada intocada como otimização), valor de função materializado e chamada indireta real desde a Fase 242 (handle callable de 1 palavra, descritor `{code_ptr, env_ptr}`, `call *reg` igual no interpretador e no backend nativo); **entregue na Fase 243** a captura léxica automática, imutável e por valor: mesmo descritor da Fase 242, agora com `env_ptr` para bloco alocado em heap (`pinker_alocar`) com o snapshot das capturas, `__env` uniforme como argumento oculto final em toda chamada indireta (closure ou wrapper sintético `__fnref_env_<nome>` de função top-level), resolução léxica preguiçosa com propagação transitiva por closures aninhadas, sombreamento correto por parâmetro/local e captura imutável (reatribuição rejeitada); idioma de chamada imediata da Fase 225 preservado sem captura; fora: captura mutável, captura por referência, coleta de lixo do ambiente, captura de tipo maior que uma palavra, equality de callables, cast callable↔inteiro, lista/mapa de callables, callable genérico não monomorfizado, objetos de trait e bound methods |

#### Obrigações derivadas da Fase 243 — callables e ambientes

- Toda criação de closure capturante aloca um ambiente no heap, independentemente de a closure escapar ou não. No recorte atual, esses ambientes não são recuperados e permanecem vivos até o término do processo.
- Qualquer fase que introduza captura mutável, captura por referência, recuperação de ambientes ou capturas maiores que uma palavra deve definir antes o contrato de ownership e lifetime: quem possui o ambiente; como cópias, parâmetros e retornos de handles compartilham esse ambiente; quando ele pode ser liberado; e como double free, use-after-free e ponteiros para pilha expirada são impedidos.
- `alocar`/`liberar` fornece mecanismo de memória, não a política de ownership. A entrega do item 13 não pode ser tratada, isoladamente, como solução automática do lifetime de closures.
- No recorte atual, em que cada captura ocupa exatamente uma palavra, `bytes_solicitados = quantidade_de_capturas * tamanho_da_palavra`, com multiplicação verificada contra overflow.
...
- `trato<Nome>` é um handle de uma palavra para um descritor `{data_ptr, vtable_ptr}` de duas palavras; o snapshot concreto é copiado por valor na materialização e materializações distintas não compartilham snapshot.
- Vtables são estáticas em `.rodata`, deduplicadas por `(trato, tipo concreto)` e ordenadas pela declaração do trato. O despacho carrega o slot e faz chamada indireta com receiver primeiro pela ABI SysV, sem `__env`.
- O lifetime atual é monotônico e explícito: snapshots e descritores não são liberados, coletados ou contados; qualquer mudança futura de ownership/lifetime precisa de contrato próprio.
...
| # | Item | Inspiração |
|---|---|---|
| 15 | Union types / tagged unions | C, C++, Rust — **entregue na Fase 248** com `uniao<T...>`, canonicalização, tags determinísticas, injeção explícita e `encaixe` exaustivo; a revisão humana da PR #411 foi respondida integralmente e a PR está mergeada: `encaixe` é construto tipado com tags exclusivamente do registry canônico e a injeção casa o membro por identidade semântica resolvida (`ResolvedTypeId`), distinta da representação operacional (`TypeIR`), com apelidos transparentes em profundidade e sem desempate por primeira ocorrência; os payloads estruturais são suportados (HR3) com classificação exaustiva de representação, snapshot alinhado multi-palavra no descritor, ABI de criação e extração por endereço e orçamentos explícitos, sem o fallback `(8, 8)`. O endurecimento do runtime nativo pós-PR #411 foi concluído e mergeado pela PR #412, sem abrir fase funcional nova |
...
**Relação com a Faixa 3.** O item 13 (alocador como superfície de linguagem, `alocar`/`liberar`) é distinto e continua na Faixa 3: o Eixo B entrega o alocador **interno** do runtime; a exposição na linguagem vem depois, sobre ele. Essa superfície fornece o mecanismo de alocação e liberação, mas o ownership e o lifetime dos ambientes de closure permanecem um contrato separado e obrigatório.
