...
/// SHA-256 na forma canônica, delegado ao núcleo compartilhado da Parte E2.
///
/// Era uma cópia manual do algoritmo. Duplicar SHA-256 no repositório é o que o
/// contrato proíbe, e uma cópia de teste diverge tão silenciosamente quanto uma
/// de produção.
...
fn catalogo_real_cartografa_o_guardiao_pinker_da_onda_9() {
    ...
    assert_eq!(index.regions.len(), 597);
    ...
    assert_eq!(by_layer("evidencia"), 295);
...
fn camada_runtime_cartografa_o_runtime_nativo() {
    ...
    for key in hosted_runtime_keys {
        let region = catalog
            .region(key)
            .unwrap_or_else(|| panic!("chave runtime hospedada ausente no catálogo: {key}"));
        assert!(region.file.starts_with("src/"));
    }

    let expected_runtime_count = expected_runtime_keys.len() + hosted_runtime_keys.len();
    assert_eq!(
        runtime_regions.len(),
    ...
    assert_eq!(
        runtime_regions
            .iter()
            .filter(|r| r.file == "runtime/pinker_rt/src/lib.rs")
            .count(),
        expected_runtime_keys.len()
    );
...
fn onda_8c_cartografa_evidencias_semanticas() {
    ...
    assert_eq!(test_count, 393, "contagem de #[test] inesperada em {file}");
...
fn onda_8e_cartografa_evidencias_da_execucao_interpretada() {
    ...
    let historical_evidence_total = catalog
        ...
        .count();
    assert_eq!(
        historical_evidence_total, 198,
        "o estado histórico da Onda 8E deve conter 159 regiões de evidência (111 anteriores + 47 da Onda 8E + 1 da Fase 243 em tests/semantic_tests.rs), mais as evidências adultas D9, D10 e a política de recurso D13; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a evidência da identidade genérica injetiva da #476; mais a região de evidência da paridade de argumento nomeado da #492"
    );
    for previous in [
        "evidencia.ir.lowering-programa",
        "evidencia.select.blocos-e-terminadores",
...
fn onda_8f_cartografa_evidencias_do_backend_textual() {
    ...
    assert_eq!(
        historical_catalog_total, 421,
        "o estado histórico da Onda 8F deve totalizar 349 regiões (348 + 1 região nova de Fase 243 em src/ast.rs; +6 regiões das correções da revisão humana da PR #411; +2 regiões de HR4 em src/ir.rs; +1 região de HR3 em src/union_payload.rs; +3 regiões do endurecimento pós-PR #411); mais as regiões da continuação pós-PR #411 (simetria das formas de chamada); mais duas regiões da portabilidade do contrato de SIGPIPE (a evidência interna do runtime e a das famílias de subprocesso); mais quatro regiões do hotfix da atribuição de símbolo em `sussurro` (o leitor de ELF, o invariante de artefato e as duas de evidência); mais as regiões da paridade de contabilidade de uniões (a matriz dos dois domínios de storage do interpretador e a evidência externa de paridade entre os backends); mais a evidência D9 de fatiar_verso; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais as duas regiões da #476; mais a região de evidência da paridade de argumento nomeado da #492"
    );
...
fn onda_8g_cartografa_evidencias_do_backend_s_textual() {
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        206,
        "o estado histórico da Onda 8G deve totalizar 172 regiões de evidência; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a evidência da #476; mais a região de evidência da paridade de argumento nomeado da #492"
    );
...
fn onda_8_convergencia_fecha_cadeia_8a_8j() {
    ...
    assert_eq!(
        historical
            ...
            .count(),
        243,
        "a convergência da Onda 8 exige exatamente 203 regiões de evidência; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a evidência da #476; mais a região de evidência da paridade de argumento nomeado da #492"
    );
    assert_eq!(
        historical
            .iter()
...
fn onda_8h_cartografa_evidencias_da_toolchain_externa() {
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        216,
        "o estado histórico da Onda 8H deve totalizar 182 regiões de evidência; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a evidência da #476; mais a região de evidência da paridade de argumento nomeado da #492"
    );
...
fn onda_8i_cartografa_evidencias_e_paridade_do_backend_nativo() {
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        230,
        "o estado histórico da Onda 8I deve totalizar 196 regiões de evidência; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a evidência da #476; mais a região de evidência da paridade de argumento nomeado da #492"
    );
...
fn onda_8j_cartografa_evidencias_internas_do_runtime() {
    ...
    assert_eq!(
        expected_keys.len(),
        12,
        "as doze chaves 8J devem ser únicas"
    );
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        243,
        "a Onda 8J deve totalizar 203 regiões de evidência; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a região de evidência da paridade de argumento nomeado da #492"
    );
    assert_eq!(
        catalog
            .regions
    ...
    assert_eq!(
        evidencia_runtime_keys
            ...
            .collect::<HashSet<_>>(),
        expected_keys,
        "o prefixo evidencia.runtime. preserva as chaves 8J e a evidência externa de identidade"
    );
    ...
    let bloco_fim = ordered[expected_regions.len() - 1].2;
    assert_eq!(
    ...
    assert_eq!(
        ordered[expected_regions.len() - 1].3,
        central_lines.len(),
        "o marcador de fim da última região 8J é a última linha do arquivo"
    );

    // Marcadores: 20 regiões de produção (Fase 248 + as duas da família JSON da
    // Parte E1) + 12 de evidência (8J + HR3 + a evidência interna JSON).
    assert_eq!(
        central_source.matches("@pinker-nav:start ").count(),
        33,
        "{central} deve conter 33 marcadores de início (21 produção + 12 evidência)"
    );
    assert_eq!(
        central_source.matches("@pinker-nav:end ").count(),
        33,
        "{central} deve conter 33 marcadores de fim (21 produção + 12 evidência)"
    );
    assert_eq!(
        central_source
    ...
    // contam como helper Rust com corpo. A paridade de contabilidade de uniões
    ...
    for expected in [
        ...
        "fn script_processo(",
        "fn rss_atual_bytes(",
        "fn paginas_residentes(",
        "fn core_limit(",
        "fn verso_de(",
        "fn identidades_publicas_registradas(",
        "fn orcamento_de_unioes(",
        "fn filho_contabilidade_uniao(",
    ] {
        assert!(
            helpers
                .iter()
    ...
    assert_eq!(
        ...
        "as chaves runtime.* devem incluir processos, falhas, filesystem e identidade"
    );
    let central_production_regions: Vec<_> = production_regions
        .iter()
        .copied()
        .filter(|region| region.file == central)
        .collect();
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        33,
        "{central} deve registrar 33 regiões (21 de produção + 12 de evidência), com as três da família JSON da Parte E1"
    );
    ...
    // dezessete regiões produtivas. Contagem estrutural; não resolve macros.
    ...
    for &linha in &definicoes {
        let donos = central_production_regions
            .iter()
    ...
    // 115 símbolos de ABI: 107 `extern "C" fn` nomeadas na produção mais os 8
    ...
    for region in &central_production_regions {
    ...
    assert_eq!(
        com_evidencia, 12,
        "dez regiões de produção têm ao menos um teste interno relacionado"
    );
    ...
    assert_eq!(
        ...
        "os domínios de produção sem teste interno relacionado divergiram"
    );
...
fn capsula_trama_query_cartografa_suporte_e_dez_testes() {
    ...
    assert_eq!(
        ...
        "5aba57a55a586706373fe3e816c71eee5c8a29ed57da724fd64a5994543738c4",
        "retirar marcadores deve reconstruir exatamente a fonte-base"
    );
