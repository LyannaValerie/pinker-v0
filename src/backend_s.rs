...
///
/// Escopo deliberadamente mínimo para a Fase 129:
...
/// - composto mínimo conservador: base homogênea `seta<bombom>` com `deref_store` mínimo e abertura heterogênea camada 1 para `ninho` apenas em leitura auditável de campo escalar `u32` via `deref_load` + offset explícito;
    ...
    for function in &selected.functions {
        ...
        for param in &function.params {
            ...
            if !is_external_param_type(ty) {
                return Err(err(
                "subset externo montável (Fase 129) aceita parâmetro `bombom`, `u32`, `u64` ou `seta<T>` no recorte conservador (inteiros mais largos + composto mínimo homogêneo/heterogêneo camada 1 + `quebrar`/`continuar` em loop mínimo)",
                ));
            }
        }
        for local in &function.locals {
            ...
            if !is_external_local_type(ty) {
                return Err(err(&format!(
                    "subset externo montável (Fase 129) só aceita local `bombom`, `u32`, `u64` ou `seta<T>`; '{}' é '{}'",
                    local,
                    ty.name()
                )));
            }
                ...
                match inst {
                    ...
                    SelectedInstr::DerefLoad {
                        ...
                        if !is_external_deref_load_type(ty) {
                            return Err(err(
                                "subset externo montável (Fase 129) aceita `deref_load` apenas no recorte mínimo `bombom`/`u32` (camada 1 conservadora de `ninho` heterogêneo + legado homogêneo)",
                            ));
                        }
                        if *is_volatile {
                            return Err(err(
                                "subset externo montável (Fase 129) ainda não suporta caminho `fragil` no acesso indireto externo",
                            ));
                        }
                    ...
                    SelectedInstr::DerefStore {
                        ...
                        if *ty != TypeIR::Bombom {
                            return Err(err(
                                "subset externo montável (Fase 129) aceita `deref_store` apenas para `seta<bombom>` (recorte homogêneo conservador preservado)",
                            ));
                        }
                        if *is_volatile {
                            return Err(err(
...
fn is_external_deref_load_type(ty: &TypeIR) -> bool {
    *ty == TypeIR::Bombom || *ty == TypeIR::U32
}
