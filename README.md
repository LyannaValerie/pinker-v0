...
- coleções dinâmicas no recorte versionado: `lista<bombom>`, `lista<verso>`, `lista<Leque>` com fachada genérica de lista, e `mapa<K,V>` nas quatro combinações públicas `verso`/`bombom` com fachada genérica `mapa_criar`/`mapa_definir`/`mapa_obter`/`mapa_tem`/`mapa_tamanho`/`mapa_remover`, inclusive quando o mapa vem de expressão tipada; listas/mapas versionados têm paridade nativa no recorte compatível (Fases 149–152, 211, 216–217, 233, 235)
- funções genéricas de usuário no Bloco 20, Eixo A: `carinho nome<T>(valor: T) -> T { ... }` com chamada explícita `nome<bombom>(...)`, monomorfização por uso concreto, suporte a `T` em parâmetro/retorno/anotações locais e `lista<T>` no recorte público de listas, com paridade interpretador × nativo (Fase 236)
- traits/interfaces estáticos no Bloco 20, Eixo A: declaração `trato Nome { carinho metodo(...); }`, chamada por método `valor.metodo(...)`, `impl Trato para Tipo { ... }` com receiver explícito, resolução nominal do método pelo tipo do receiver antes do fallback global, receiver nominal `ninho` no recorte opaco atual, validação de cobertura completa do contrato, múltiplos contratos por tipo e desambiguação explícita de métodos homônimos com `Trato.metodo(valor, ...)`, com paridade interpretador × nativo (Fases 226–230, 232, 234)
- closures/funções anônimas não capturantes no Bloco 20, Eixo A: literal `carinho(x: bombom) -> bombom { ... }` em expressão, chamável diretamente, e função local tipada `nova f: carinho(bombom) -> bombom = carinho(x: bombom) -> bombom { ... };` chamável como `f(21)`, com geração de função sintética top-level e paridade interpretador × nativo (Fases 225 e 238)
- error handling estruturado no Bloco 20, Eixo A: construto `tentar resultado { sucesso Resultado.Ok(v) { ... } falha Resultado.Erro(e) { ... } }`, apoiado em leques de resultado declarados pelo usuário, desugaring para controle/extração de leque já existente e paridade interpretador × nativo desde a entrega (Fases 223–224), incluindo propagação explícita com `propagar expr como Resultado.Ok(v) senao Resultado.Erro(e);`, valor de sucesso nomeado disponível após a propagação (Fase 231) e forma curta `propagar? expr como Resultado.Ok(v);` quando há uma única variante de falha inferível com uma carga (Fase 237)
...
- FFI geral, objetos de trait, vtables/dynamic dispatch, coerções de trait, default methods, métodos homônimos entre contratos do mesmo tipo, overloading amplo de contratos, closures capturantes, funções como valores armazenáveis/passáveis além do alias local tipado da Fase 238 e chamada indireta real
