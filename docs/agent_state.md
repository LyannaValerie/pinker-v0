...
## 5. Estado corrente
- Fase funcional atual: **126 — `quebrar` / `continuar` (camada 1 conservadora) no backend nativo externo**.
- Fase funcional anterior: **125 — comparações ampliadas (camada 4 conservadora) no backend nativo externo**.
...
4. Próxima rodada normal esperada: continuar o item **10.3 (`quebrar`/`continuar`)** em camada conservadora seguinte, sem abrir 10.4 junto, sem inverter `ninho`/`virar` e sem antecipar `verso`.
