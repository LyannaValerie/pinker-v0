...
- As Fases 120–127 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`), comparações ampliadas mínimas (`!=`, `>`, `<=` e `>=`) e dois degraus conservadores do recorte externo auditável de `quebrar`/`continuar` no caminho de `sempre que` (camada 1 mínima + camada 2 com aninhamento mínimo controlado).
