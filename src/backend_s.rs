...
///
/// Escopo deliberadamente mínimo para a Fase 132:
    ...
    for function in &selected.functions {
        ...
        for param in &function.params {
            ...
            if !is_external_param_type(ty) {
                return Err(err(
                "subset externo montável (Fase 132) aceita parâmetro `bombom`, `u32`, `u64` ou `seta<T>` no recorte conservador (inteiros mais largos + composto mínimo homogêneo/heterogêneo camada 1 + `quebrar`/`continuar` em loop mínimo)",
                ));
            }
        }
        for local in &function.locals {
            ...
            if !is_external_local_type(ty) {
                return Err(err(&format!(
                    "subset externo montável (Fase 132) só aceita local `bombom`, `u32`, `u64` ou `seta<T>`; '{}' é '{}'",
                    local,
                    ty.name()
                )));
            }
                ...
                match inst {
                    ...
                    SelectedInstr::DerefLoad {
                        ...
                        if !is_external_deref_load_type(ty) {
                            return Err(err(
                                "subset externo montável (Fase 132) aceita `deref_load` apenas no recorte mínimo `bombom`/`u32`/`u64` (camada 4 conservadora de `ninho` heterogêneo + legado homogêneo)",
                            ));
                        }
                        if *is_volatile {
                            return Err(err(
                                "subset externo montável (Fase 132) ainda não suporta caminho `fragil` no acesso indireto externo",
                            ));
                        }
                    ...
                    SelectedInstr::DerefStore {
                        ...
                        if !is_external_deref_store_type(ty) {
                            return Err(err(
                                "subset externo montável (Fase 132) aceita `deref_store` apenas no recorte mínimo `bombom`/`u32`/`u64` (camada 4 conservadora de `ninho` heterogêneo + legado homogêneo)",
                            ));
                        }
                        if *is_volatile {
                            return Err(err(
