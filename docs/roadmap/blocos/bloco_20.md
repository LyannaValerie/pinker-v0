...
| 3 | Generics mínimos (`lista<T>`, `mapa<K,V>`) | C++, TS, C# | eliminar monomorphização manual — **expandido nas Fases 211, 233 e 235**: `lista<T>` com T = leque + 7 intrínsecas genéricas sobre qualquer lista; `mapa<K,V>` nas quatro combinações públicas `verso`/`bombom` com fachada genérica e despacho pelo tipo da expressão de mapa; fora: funções genéricas de usuário, generics em `leque`/`ninho` e coleções heterogêneas |
