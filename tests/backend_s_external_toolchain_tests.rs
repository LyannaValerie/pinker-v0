...
fn asm_s_external_subset_falha_parametro_nao_bombom() {
    ...
    assert!(err.to_string().contains(
        "subset externo montável (Fase 121) aceita parâmetro `bombom`, `u32`, `u64` ou `seta<bombom>`"
    ));
}
...
fn asm_s_external_subset_fase84_matriz_fronteira_auditavel() {
    ...
    for code in casos_garantidos {
        ...
        assert!(asm.contains("# pinker v0 external toolchain subset (fase 121"));
    }
...
fn asm_s_external_subset_fase117_recusa_local_composto_fora_da_camada2() {
    ...
    let err = render_backend_s_external_subset(code).unwrap_err();
    assert!(err.to_string().contains(
        "subset externo montável (Fase 121) só aceita local `bombom`, `u32`, `u64` ou `seta<bombom>`"
    ));
}
...
#[test]
fn asm_s_external_subset_fase121_recusa_parametro_i16_fora_do_recorte() {
    let code = r#"
pacote main;
carinho soma_i16(a: i16) -> bombom {
    mimo 0;
}
...
#[test]
fn asm_s_external_subset_fase121_recusa_parametro_i32_fora_do_recorte() {
    let code = r#"
pacote main;
carinho soma_i32(a: i32) -> bombom {
    mimo 0;
}
carinho principal() -> bombom {
    mimo soma_i32(1);
