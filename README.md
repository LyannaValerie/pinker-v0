...
- traits/interfaces estáticos iniciais no Bloco 20, Eixo A: declaração `trato Nome { carinho metodo(...); }`, chamada por método `valor.metodo(...)`, `impl Trato para Tipo { ... }` com receiver explícito e resolução nominal do método pelo tipo do receiver antes do fallback global, com paridade interpretador × nativo (Fases 226–228)
...
- FFI geral, objetos de trait, vtables/dynamic dispatch, coerções de trait, closures capturantes, funções como valores armazenáveis/passáveis e tipos-função públicos
