...
- As Fases 120–134 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`), comparações ampliadas mínimas (`!=`, `>`, `<=` e `>=`), três degraus conservadores do recorte externo auditável de `quebrar`/`continuar` no caminho de `sempre que`, quatro camadas conservadoras do recorte heterogêneo mínimo de `ninho` em 10.4 (`u32` e `u64` em leitura/escrita e composição mínima auditável no mesmo `ninho`) e as camadas 1 e 2 conservadoras de `virar` em 10.5 (casts explícitos mínimos `u32 -> u64` e `u64 -> u32` no backend externo, sem coerções implícitas e sem sistema geral de casts).
