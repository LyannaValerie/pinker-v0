...
| 4 | Traits / interfaces mínimas | Rust, TS, C# | polimorfismo sem herança, contratos de driver — **expandido nas Fases 226–230**: `trato` como contrato estático, chamada por método, `impl Trato para Tipo { ... }` com receiver explícito, resolução nominal pelo tipo do receiver, suporte a receiver nominal `ninho` no recorte opaco atual e cobertura completa do contrato; fora: objetos de trait, vtables, dynamic dispatch, coerções, default methods e overloading amplo |
