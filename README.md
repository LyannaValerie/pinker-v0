...
- traits/interfaces estáticos iniciais no Bloco 20, Eixo A: declaração `trato Nome { carinho metodo(...); }` validando função compatível, chamada por método `valor.metodo(...)` como açúcar para chamada direta e `impl Trato para Tipo { ... }` com receiver explícito como agrupamento nominal inicial, com paridade interpretador × nativo (Fases 226–227)
...
- error handling estruturado inicial no Bloco 20, Eixo A: construto `tentar resultado { sucesso Resultado.Ok(v) { ... } falha Resultado.Erro(e) { ... } }`, apoiado em leques de resultado declarados pelo usuário, desugaring para controle/extração de leque já existente e paridade interpretador × nativo desde a entrega (Fases 223–224), incluindo propagação explícita com `propagar expr como Resultado.Ok(v) senao Resultado.Erro(e);`
...
- FFI geral, traits com implementação por tipo/dynamic dispatch, closures capturantes, funções como valores armazenáveis/passáveis e tipos-função públicos
