...
- As Fases 120–124 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`) e comparações ampliadas mínimas (`!=`, `>` e `<=`) no caminho externo.
