...
- As Fases 120–129 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`), comparações ampliadas mínimas (`!=`, `>`, `<=` e `>=`), três degraus conservadores do recorte externo auditável de `quebrar`/`continuar` no caminho de `sempre que` e o primeiro recorte heterogêneo mínimo de `ninho` (camada 1 conservadora de 10.4, sem abrir sistema geral de compostos).
