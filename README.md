...
Status documental corrente: **Fase 132 ampliou o item 10.4 do Bloco 10** com camada 4 conservadora de `ninho` heterogêneo mínimo no backend nativo externo (composição heterogênea mínima auditável no mesmo `ninho` com leitura/escrita de campos `u32`/`u64` via offset explícito), mantendo tese de **cobertura semântica do backend nativo** (sem prometer backend pleno).
