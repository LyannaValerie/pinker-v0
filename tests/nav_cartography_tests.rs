...
fn catalogo_real_cartografa_o_guardiao_pinker_da_onda_9() {
    ...
    assert_eq!(index.regions.len(), 562);
    ...
    assert_eq!(by_layer("evidencia"), 277);
    assert_eq!(by_layer("runtime"), 17);
...
fn onda_8e_cartografa_evidencias_da_execucao_interpretada() {
    ...
    let historical_evidence_total = catalog
        ...
        .count();
    assert_eq!(
        historical_evidence_total, 182,
        "o estado histórico da Onda 8E deve conter 159 regiões de evidência (111 anteriores + 47 da Onda 8E + 1 da Fase 243 em tests/semantic_tests.rs), mais as evidências adultas D9, D10 e a política de recurso D13"
    );
    for previous in [
        "evidencia.ir.lowering-programa",
        "evidencia.select.blocos-e-terminadores",
...
fn onda_8j_cartografa_evidencias_internas_do_runtime() {
    ...
    let expected_regions: [(&str, &str, usize); 11] = [
        ("evidencia.runtime.memoria-alocador", "memoria", 20),
    ...
    assert_eq!(
        expected_ownership,
        vec![20usize, 9, 6, 3, 3, 3, 5, 4, 3, 1, 14],
        "o ownership aprovado da Onda 8J é [20,9,6,3,3,3,5,4,3,1,14]"
    );
    assert_eq!(
        expected_ownership.iter().sum::<usize>(),
        71,
        "a soma do ownership 8J deve ser 71"
    );
    ...
    assert_eq!(
        test_lines.len(),
        71,
        "{central} deve manter exatamente 71 testes"
    );
    assert!(
        test_lines
            .iter()
    ...
    assert_eq!(
        ...
        "as dezessete chaves runtime.* devem incluir o descritor de união e as superfícies falíveis da Parte B"
    );
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        28,
        "{central} deve registrar 25 regiões (17 de produção + 8 de evidência)"
    );
    ...
    assert_eq!(
        definicoes.len(),
        247,
        "a produção do runtime deve manter 247 definições textuais"
    );
    ...
    assert_eq!(
        com_evidencia, 9,
        "nove regiões de produção têm ao menos um teste interno relacionado"
    );
