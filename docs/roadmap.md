...
**Status**: bloco ativo; a Fase 161 concluiu 15.1 no recorte mínimo conservador, a Fase 162 aplicou a correção conservadora de portabilidade em testes/exemplos e a Fase 163 concluiu 15.2 com captura mínima de stdout como `verso`. A Doc-30 refinou a escada interna restante do bloco para subdegraus menores e mais auditáveis, sem abrir fase funcional nova. Dependência satisfeita: Bloco 14 foi encerrado por suficiência conservadora na Doc-29.
...
3. **15.3 — captura mínima de stderr como `verso`**: abrir `capturar_stderr(comando) -> verso` no mesmo desenho conservador de 15.2, com comando/caminho único, sem shell implícito, captura apenas de stderr textual e sem mistura com stdout; precisa de processo externo (15.1) e da disciplina de captura aberta em 15.2; próximo subitem funcional correto do bloco.
4. **15.4 — entrada mínima por stdin textual para processo externo**: permitir alimentar um processo externo com um `verso` simples em stdin no recorte mínimo, sem shell implícito, sem stream amplo, sem TTY e sem sessão interativa; precisa da base de execução de processo (15.1) e da disciplina de captura já consolidada em 15.2/15.3.
5. **15.5 — pipe mínimo entre dois processos**: compor dois processos no menor recorte auditável, com stdout do primeiro alimentando stdin do segundo, sem shell implícito, sem cadeia longa e sem gramática ampla de shell; precisa de stdout (15.2) + stdin textual para processo (15.4); fecha o bloco no recorte pequeno de linguagem-cola sistêmica.
