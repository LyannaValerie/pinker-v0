...
- Síntese atual do Bloco 18: famílias públicas iniciais aceitas (`texto`, `arquivo`, `caminho`, `processo`, `tempo`, `ambiente`, `acaso`), domínios provisórios explícitos (`colecao`, `formato`), formalização de `tempo` como família exemplar do bloco sobre `tempo_unix` e `formatar_tempo_unix`, política documental explícita para a futura superfície por família, classificação canônica de domínio interno por intrínseca e preparação canônica da futura resolução qualificada por família.
- Limite canônico de 18.2: a Pinker ainda não possui `familia.intrinseca`, `trazer familia;`, `trazer familia.intrinseca;`, resolução qualificada operacional por família nem qualquer reorganização funcional da engine ligada a famílias.
