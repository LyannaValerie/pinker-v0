...
- Capturas multi-palavra exigem layout, tamanho e alinhamento explícitos, com a mesma representação validada no interpretador, no IR, na ABI SysV e no backend nativo antes de sua liberação como superfície pública.
...
- A suíte de sensibilidade de ambientes deve detectar underallocation diretamente. O gate futuro deve comprovar `bytes_solicitados == quantidade_de_capturas * tamanho_da_palavra` por alocador instrumentado, metadado de tamanho, canário ou caso que atravesse a classe de arredondamento; observar apenas o resultado funcional da closure não basta.
