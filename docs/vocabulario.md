...
- As Fases 120–132 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`), comparações ampliadas mínimas (`!=`, `>`, `<=` e `>=`), três degraus conservadores do recorte externo auditável de `quebrar`/`continuar` no caminho de `sempre que` e quatro camadas conservadoras do recorte heterogêneo mínimo de `ninho` em 10.4 (`u32` e `u64` em leitura/escrita e composição mínima auditável no mesmo `ninho`, sem abrir sistema geral de compostos/layout).
