...
| 3 | Generics mínimos (`lista<T>`, `mapa<K,V>`) | C++, TS, C# | eliminar monomorphização manual — **expandido nas Fases 211, 233, 235 e 236**: `lista<T>` com T = leque + 7 intrínsecas genéricas sobre qualquer lista; `mapa<K,V>` nas quatro combinações públicas `verso`/`bombom` com fachada genérica e despacho pelo tipo da expressão de mapa; funções genéricas de usuário com chamada explícita `nome<T>(...)` e monomorfização por uso concreto; fora: inferência de tipo, generics em `leque`/`ninho`, bounds/especialização e coleções heterogêneas |
...
| 5 | Error handling estruturado | C#, Python, Rust | recuperação sem abort, relatório de erros do compilador — **expandido nas Fases 223–224, 231 e 237**: `tentar` com braços `sucesso`/`falha`, `propagar` para retorno antecipado explícito sobre leque de resultado declarado pelo usuário, ligação nomeada do valor de sucesso para continuação e forma curta `propagar?` quando há uma única variante de falha inferível com uma carga, com lowering nativo desde a entrega; fora: biblioteca padrão de `Resultado<T,E>` e integração automática com erros de runtime |
| 6 | Closures / funções anônimas | Rust, TS, Python | callbacks, iteradores, handlers — **expandido nas Fases 225 e 238**: literal `carinho (...) -> tipo { ... }` não capturante em expressão, gerado como função sintética, e função local tipada `nova f: carinho(...) -> tipo = carinho(...) -> tipo { ... };` chamável por nome local, executados também no backend nativo; fora: captura de ambiente, passagem/retorno de função, função armazenável ampla e chamada indireta |
