...
- traits/interfaces estáticos iniciais no Bloco 20, Eixo A: declaração `trato Nome { carinho metodo(...); }` validando função top-level compatível, combinada com chamada por método `valor.metodo(...)` como açúcar para chamada direta com paridade interpretador × nativo (Fase 226)
