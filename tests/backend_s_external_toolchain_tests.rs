...
fn asm_s_external_subset_emite_main_montavel() {
    ...
    assert!(out.contains("# pinker v0 external toolchain subset (fase 73, linux x86_64)"));
...
fn asm_s_external_subset_falha_clara_fora_do_subset() {
    let code = include_str!("../examples/fase73_backend_externo_if_invalido.pink");

    let err = render_backend_s_external_subset(code).unwrap_err();
    assert!(err
        .to_string()
        .contains("subset externo montável (Fase 73)"));
}
...
fn unique_temp_dir() -> std::path::PathBuf {
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    std::env::temp_dir().join(format!("pinker_phase55_{}", nanos))
}
