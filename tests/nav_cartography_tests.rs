...
fn catalogo_real_cartografa_o_guardiao_pinker_da_onda_9() {
    ...
    assert_eq!(index.regions.len(), 604);
    ...
    assert_eq!(by_layer("evidencia"), 296);
...
fn onda_8j_cartografa_evidencias_internas_do_runtime() {
    ...
    assert_eq!(
        catalog
            ...
            .count(),
        244,
        "a Onda 8J deve totalizar 203 regiões de evidência; mais a evidência da Parte B1 de identidade de `Resultado` produzida pelo runtime; mais a região de evidência da paridade de argumento nomeado da #492; mais a região de evidência da autoridade nativa de símbolos da #497"
    );
    assert_eq!(
        catalog
            .regions
