...
- **Fase 128 concluída (camada 3 conservadora do item 10.3)**: backend nativo externo ampliou o mesmo recorte com composição mínima auditável de três níveis de `sempre que` (externo/meio/interno), mantendo alvos distintos de `quebrar`/`continuar` no fluxo externo sem abrir controle de fluxo geral, sem abrir 10.4 e sem prometer suporte amplo de aninhamento.
...
Estes itens estão organizados na trilha canônica dos Blocos 12–16 e não devem competir com o foco ativo do Bloco 11.
- sistema de módulos tipado (`ninho`/`apelido` exportáveis) → **Bloco 12**;
- coleções (`lista`, `mapa`), iteração confortável, aleatoriedade básica → **Bloco 13**;
- formatação simples, CSV, JSON, datas e tempo → **Bloco 14**;
- execução de processos externos, captura de saída, integração stdin/stdout/stderr → **Bloco 15**;
- REPL, linguagem-cola, ferramenta cotidiana madura → **Bloco 16**.
...
**Dependência obrigatória**: Bloco 12 concluído — tipos exportáveis permitem que os tipos de coleção sejam compartilhados entre módulos de forma utilizável.
...
**Status**: bloco ativo; a Fase 161 concluiu 15.1 no recorte mínimo conservador, a Fase 162 aplicou a correção conservadora de portabilidade em testes/exemplos, a Fase 163 concluiu 15.2 com captura mínima de stdout como `verso`, a Fase 164 concluiu 15.3 com captura mínima de stderr como `verso` e a Fase 165 concluiu 15.4 com entrada mínima por stdin textual para processo externo. A Doc-30 refinou a escada interna restante do bloco para subdegraus menores e mais auditáveis antes desta continuidade funcional. Dependência satisfeita: Bloco 14 foi encerrado por suficiência conservadora na Doc-29.
...
2. **15.2 — captura de saída de comandos (Fase 163, concluída no recorte mínimo)**: capturar stdout de processo externo como `verso` via `capturar_stdout(comando) -> verso`, mantendo comando/caminho único, sem shell implícito, com UTF-8 estrito e sem stderr separado; precisa de processo externo (15.1); base para os próximos subdegraus do bloco, mas ainda não equivale a integração completa de processos.
3. **15.3 — captura mínima de stderr como `verso` (Fase 164, concluída no recorte mínimo)**: abre `capturar_stderr(comando) -> verso` no mesmo desenho conservador de 15.2, com comando/caminho único, sem shell implícito, captura apenas de stderr textual e sem mistura com stdout; precisa de processo externo (15.1) e da disciplina de captura aberta em 15.2; fica concluído no recorte mínimo conservador.
4. **15.4 — entrada mínima por stdin textual para processo externo (Fase 165, concluída no recorte mínimo)**: permite alimentar um processo externo com um `verso` simples em stdin no recorte mínimo via `executar_com_entrada(comando, entrada) -> bombom`, sem shell implícito, sem stream amplo, sem TTY e sem sessão interativa; precisa da base de execução de processo (15.1) e da disciplina de captura já consolidada em 15.2/15.3; fica concluído no recorte mínimo conservador.
5. **15.5 — pipe mínimo entre dois processos**: compor dois processos no menor recorte auditável, com stdout do primeiro alimentando stdin do segundo, sem shell implícito, sem cadeia longa e sem gramática ampla de shell; precisa de stdout (15.2) + stdin textual para processo (15.4); próximo subitem funcional correto do bloco e fechamento do bloco no recorte pequeno de linguagem-cola sistêmica.
...
**Dependência obrigatória**: Bloco 15 concluído — REPL e linguagem-cola dependem de toda a cadeia 12–15.

**Itens do bloco (ordem interna por dependência)**:
1. **16.1 — REPL mínimo**: loop de leitura-avaliação-impressão para expressões e declarações simples da Pinker; precisa de toda a cadeia 12–15 consolidada; base para prototipagem interativa e uso fluido da linguagem-cola.
...
**Escopo deliberadamente fora do Bloco 16**: IDE integrada, debugger interativo, package manager soberano, autohospedagem, macros de compilação.
