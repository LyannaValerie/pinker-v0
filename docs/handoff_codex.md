...
## 1. Rodada atual
- **Fase 169 — argv explícito mínimo para captura de stdout (camada 2 conservadora)**.
- Fase funcional ativa corrente: **169** (15.1, 15.2, 15.3, 15.4, 15.5 e 16.1 já concluídos no recorte mínimo; 16.2 segue aberta por camadas conservadoras pequenas).
...
- A Fase 169 reduz a assimetria aberta pela Fase 168 com `argv` explícito mínimo em `capturar_stdout(comando, argv1)`.
...
- O desenho reaproveita a infraestrutura mínima já aberta no Bloco 15 e o padrão da Fase 168, sem parser amplo de shell, quoting/escaping rico, listas gerais de argumentos, PTY, sessão interativa nem expansão automática para `capturar_stderr`, `executar_com_entrada` ou `pipeline_minimo`.
- Testes cobrem caso positivo básico, fluxo composto, erro claro fora do recorte mínimo, preservação do caminho antigo `capturar_stdout(comando)` e ausência de shell implícito.
...
- Sem inflar as Fases 168 e 169: o novo `argv1` explícito não autoriza por inércia listas gerais de argumentos, quoting/escaping rico, shell mode, `stdin` interativo, PTY, job control ou expansão automática de toda a família de subprocessos.
