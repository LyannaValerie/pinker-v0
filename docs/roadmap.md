...
- O próximo bloco formal da trilha ativa é o **Bloco 12 — sistema de módulos tipado**. As fases 144 e 145 concluíram o fechamento mínimo de exportação tipada via `trazer` (`ninho` + `apelido`) no recorte conservador.
...
- **12.2 — exportação mínima de `apelido` via `trazer` (Fase 145, concluída)**: aliases de tipo declarados em módulo exportador passam a ser resolvíveis em módulo importador com `trazer`, fechando o complemento mínimo do bloco sem inflar o sistema de módulos.
