...
A Fase 186 abriu `trazer tempo;` como primeiro recorte funcional mínimo de importação por família. As Fases 187–189 ampliaram esse mesmo mecanismo para `trazer ambiente;`, `trazer acaso;` e `trazer texto;`, e a Fase 207 o concluiu para `trazer arquivo;`, `trazer caminho;` e `trazer processo;`, sempre no menor recorte auditável e sem alterar a disponibilidade global das intrínsecas. Importação seletiva e resolução qualificada operacional permanecem fora do recorte.
...
No estado funcional final de 18.6 (Fases 186–189 e 207), as 7 famílias públicas estão importáveis via `trazer familia;`: `tempo`, `ambiente`, `acaso`, `texto`, `arquivo`, `caminho` e `processo`. Os domínios provisórios `colecao` e `formato` não são importáveis.
