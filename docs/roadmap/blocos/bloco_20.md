...
| 6 | Closures / funções anônimas | Rust, TS, Python | callbacks, iteradores, handlers — **expandido nas Fases 225, 238, 239 e 242**: literal `carinho (...) -> tipo { ... }` não capturante em expressão, função local tipada `nova f: carinho(...) -> tipo = carinho(...) -> tipo { ... };` chamável por nome local e passagem estática dessa função como parâmetro por especialização direta (preservada intocada como otimização); **entregue na Fase 242** o valor de função materializado e a chamada indireta real: referência a função top-level como valor, armazenamento em variável tipada (imutável ou `muda`), passagem/retorno de callable e chamada por expressão, com handle callable de 1 palavra (`{code_ptr, env_ptr}`, descritor estático não-capturante) igual no interpretador e no backend nativo (`call *reg`), executados também no backend nativo; fora: captura de ambiente, mutação de ambiente, equality de callables, cast callable↔inteiro, lista/mapa de callables, callable genérico não monomorfizado, objetos de trait e bound methods — capturas ficam para a Fase 243 |
