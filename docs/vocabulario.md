...
- As Fases 120–130 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`), comparações ampliadas mínimas (`!=`, `>`, `<=` e `>=`), três degraus conservadores do recorte externo auditável de `quebrar`/`continuar` no caminho de `sempre que` e dois degraus conservadores do recorte heterogêneo mínimo de `ninho` em 10.4 (`u32` e `u64` em leitura via `seta<ninho>`, sem abrir sistema geral de compostos).
