...
- As Fases 120–126 no backend nativo externo também não introduziram keyword nova: abriram cobertura semântica mínima de tipos já existentes (`u32`/`u64`), comparações ampliadas mínimas (`!=`, `>`, `<=` e `>=`) e o primeiro recorte externo auditável de `quebrar`/`continuar` no caminho de loop mínimo.
