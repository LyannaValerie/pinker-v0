...
- **Eixo A — linguagem**: as 11 faixas abaixo (superfície, tipos, funções, controle, baixo nível, metaprogramação, módulos, concorrência, I/O). Executou os itens 1–3 da Faixa 1 (Fases 208–211) e, após o Eixo B, retomou os itens 5 → 6 → 4 nas Fases 223–230.
...
| 3 | Generics mínimos (`lista<T>`, `mapa<K,V>`) | C++, TS, C# | eliminar monomorphização manual — **expandido nas Fases 211 e 233**: `lista<T>` com T = leque + 7 intrínsecas genéricas sobre qualquer lista; `mapa<K,V>` nas quatro combinações públicas `verso`/`bombom` com fachada genérica; fora: funções genéricas de usuário, generics em `leque`/`ninho` e coleções heterogêneas |
| 4 | Traits / interfaces mínimas | Rust, TS, C# | polimorfismo sem herança, contratos de driver — **expandido nas Fases 226–230 e 232**: `trato` como contrato estático, chamada por método, `impl Trato para Tipo { ... }` com receiver explícito, resolução nominal pelo tipo do receiver, suporte a receiver nominal `ninho` no recorte opaco atual, cobertura completa do contrato e múltiplos contratos por tipo quando os métodos são distintos; fora: objetos de trait, vtables, dynamic dispatch, coerções, default methods, métodos homônimos entre contratos e overloading amplo |
| 5 | Error handling estruturado | C#, Python, Rust | recuperação sem abort, relatório de erros do compilador — **expandido nas Fases 223–224 e 231**: `tentar` com braços `sucesso`/`falha`, `propagar` para retorno antecipado explícito sobre leque de resultado declarado pelo usuário e ligação nomeada do valor de sucesso para continuação, com lowering nativo desde a entrega; fora: operador curto de propagação, biblioteca padrão de `Resultado<T,E>` e integração automática com erros de runtime |
