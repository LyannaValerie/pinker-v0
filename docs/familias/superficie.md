...
- possui `trazer familia;` para as 7 famílias públicas: `tempo`, `ambiente`, `acaso`, `texto`, `arquivo`, `caminho` e `processo` (Fases 186–189 e 207, recorte mínimo);
- ainda não possui `trazer familia.algo;` (importação seletiva).

Leitura correta:

- a família existe como organização canônica da superfície;
- `trazer familia;` é o mecanismo real atual de importação por família, cobrindo as 7 famílias públicas, com efeito operacional mínimo (as intrínsecas já estão disponíveis globalmente; o import é válido mas não obrigatório);
- os domínios provisórios `colecao` e `formato` não são importáveis;
