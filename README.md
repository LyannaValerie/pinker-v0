...
- iteração confortável mínima sobre `lista<bombom>` e `mapa<verso,bombom>` no Bloco 13 (camada 1 conservadora): `para cada item em lista { ... }` e `para cada chave em mapa { ... }` com variável de loop no corpo e lowering conservador por desdobramento; valor de mapa acessado via `mapa_verso_bombom_obter`; sem iteração genérica, sem pares chave/valor amplos e sem API ampla de iteradores (Fases 153 e 154)
...
- coleções dinâmicas além do recorte mínimo atual (`lista<bombom>` + `mapa<verso,bombom>`): seguem fora `lista<T>`/`mapa<K,V>` amplos, remoção arbitrária por chave, ordenação, insert/erase gerais e iteração confortável ampla (há recorte mínimo de `para cada` sobre `lista<bombom>` e `mapa<verso,bombom>` nas Fases 153 e 154)
