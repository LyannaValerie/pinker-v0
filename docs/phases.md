...
- Fase 16 — concluída
  - robustez do interpretador com endurecimento pequeno (detecção de global duplicada no runtime)
  - cobertura ampliada de testes negativos de runtime (divisão por zero, aridade, call_void, função/global inexistente, global inválida)
  - mais testes end-to-end de `--run` para globals em expressão e chamada void
