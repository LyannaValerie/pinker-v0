...
- coleções básicas além do recorte já aberto (`lista<bombom>` + `mapa<verso,bombom>` mínimos e iteração confortável mínima em `lista<bombom>`; seguem fora `lista<T>`/`mapa<K,V>` amplos, dicionário rico e iteração confortável ampla/generics);
