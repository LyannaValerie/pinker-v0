...
impl SemanticChecker {
    ...
    fn check_call_expr(
        ...
        // Fase 161 — executar_processo(comando) -> bombom
        if name == "executar_processo" {
            if args.len() != 1 {
                return Err(PinkerError::Semantic {
                    msg: format!(
                        "chamada de 'executar_processo' com aridade inválida: esperado 1, recebido {}",
                        args.len()
                    ),
                    span: expr_span,
                });
        ...
        // Fase 164 — capturar_stderr(comando) -> verso
        if name == "capturar_stderr" {
            if args.len() != 1 {
                return Err(PinkerError::Semantic {
                    msg: format!(
                        "chamada de 'capturar_stderr' com aridade inválida: esperado 1, recebido {}",
                        args.len()
                    ),
                    span: expr_span,
                });
