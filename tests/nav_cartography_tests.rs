...
fn onda_8f_cartografa_evidencias_do_backend_textual() {
    ...
    assert_eq!(
        ...
        (147_735, 11_352_636_465_658_390_838),
...
fn onda_8g_cartografa_evidencias_do_backend_s_textual() {
    ...
    assert_eq!(
        ...
        (150_680, 2_881_594_157_287_413_656),
...
fn capsula_nav_catalog_cartografa_suporte_e_seis_testes() {
    ...
    let capsule_scope: Vec<_> = catalog
        ...
        .collect();
    assert_eq!(capsule_scope.len(), 395);
    assert_eq!(
        capsule_scope
            .iter()
            .filter(|region| region.layer.as_deref() == Some("evidencia"))
    ...
    let historical: Vec<_> = capsule_scope
        ...
        .filter(|region| region.file != target_path)
        .collect();
    assert_eq!(historical.len(), 389, "conjunto histórico exato");
    ...
    assert_eq!(
        ...
        (171_010, 7_576_940_103_060_256_314)
    );
    ...
    assert_eq!(
        ...
        (172_747, 6_435_577_717_049_132_097)
    );
...
fn capsula_doc_catalog_cartografa_suporte_e_quatro_testes() {
    ...
    let capsule_scope: Vec<_> = catalog
        ...
        .collect();
    assert_eq!(capsule_scope.len(), 401);
    assert_eq!(
        capsule_scope
            .iter()
            .filter(|region| region.layer.as_deref() == Some("evidencia"))
    ...
    let merged_base: Vec<_> = capsule_scope
        ...
        .filter(|region| region.file != target_path)
        .collect();
    assert_eq!(merged_base.len(), 395, "base mergeada exata");
    assert_eq!(
        merged_base
            .iter()
            .filter(|region| region.layer.as_deref() == Some("evidencia"))
    ...
    assert_eq!(
        (full_projection.len(), fnv1a64(full_projection.as_bytes())),
        (174_412, 14_866_948_958_847_813_146),
...
fn onda_8h_cartografa_evidencias_da_toolchain_externa() {
    ...
    assert_eq!(
        ...
        (153_541, 412_130_536_503_693_906),
...
fn onda_8i_cartografa_evidencias_e_paridade_do_backend_nativo() {
    ...
    assert_eq!(
        ...
        (160_050, 12_242_573_290_276_517_660),
...
fn onda_8j_cartografa_evidencias_internas_do_runtime() {
    ...
    assert_eq!(
        ...
        (168_105, 13_433_846_285_802_958_810),
...
fn capsula_trama_query_cartografa_suporte_e_dez_testes() {
    ...
    let predecessor: Vec<_> = predecessor_catalog
        ...
        .filter(|region| region.file != target_path)
        .collect();
    assert_eq!(predecessor.len(), 401, "base predecessora exata");
    assert_eq!(
        predecessor
            .iter()
            .filter(|region| region.layer.as_deref() == Some("evidencia"))
    ...
    let post_nav: Vec<_> = predecessor
        ...
        .collect();
    assert_eq!(post_nav.len(), 395, "conjunto pós-nav-catalog exato");
    ...
    assert_eq!(
        (full_projection.len(), fnv1a64(full_projection.as_bytes())),
        (176_898, 947_106_060_245_574_278),
...
fn onda_pink_agente_a_cartografa_nucleo_e_primeiro_dogfood() {
    ...
    assert_eq!(predecessor.len(), 410); // 407 + 1 região nova de Fase 243 em src/ast.rs
    let predecessor_projection = stable_region_projection(predecessor.iter().copied());
    assert_eq!(
        (
            predecessor_projection.len(),
    ...
    for (regions, expected) in [
        (
            post_query.as_slice(),
            (401, 174_412, 14_866_948_958_847_813_146),
        ),
        (
            post_nav.as_slice(),
            (395, 172_747, 6_435_577_717_049_132_097),
        ),
        (wave_8.as_slice(), (389, 171_010, 7_576_940_103_060_256_314)),
    ] {
    ...
    assert_eq!(
        (full_projection.len(), fnv1a64(full_projection.as_bytes())),
        (183_096, 7_298_228_100_718_464_582),
...
fn onda_pink_agente_b_verifica_integridade_e_dogfood_operacional() {
    ...
    assert_eq!(
        ...
        (187_135, 14_775_582_194_610_999_116),
    ...
    assert_eq!(
        ...
        (429, 183_096, 7_298_228_100_718_464_582)
    );
    ...
    assert_eq!(
        ...
        (410, 176_898, 947_106_060_245_574_278)
    );
    ...
    let n: Vec<_> = d
        ...
        .collect();
    for (regions, expected) in [
        (q.as_slice(), (401, 174_412, 14_866_948_958_847_813_146)),
        (d.as_slice(), (395, 172_747, 6_435_577_717_049_132_097)),
        (n.as_slice(), (389, 171_010, 7_576_940_103_060_256_314)),
    ] {
        let projection = stable_region_projection(regions.iter().copied());
        assert_eq!(
            (
...
fn onda_pink_agente_c_publica_retoma_e_cartografa_trama_restante() {
    ...
    assert_eq!(
        (full.len(), fnv1a64(full.as_bytes())),
        (191_056, 5_783_064_071_472_767_826),
    ...
    assert_eq!(
        ...
        (443, 187_135, 14_775_582_194_610_999_116)
    );
    let core = include_str!("../src/agent.rs");
    for contract in [
        "run_pr_body_check",
...
fn onda_pink_agente_d_congela_v1_sem_fechar_trama() {
    ...
    assert_eq!(catalog.regions.len(), 457);
    assert_eq!(
        catalog
            .regions
            .iter()
    ...
    assert_eq!(
        (full.len(), fnv1a64(full.as_bytes())),
        (191_508, 16_988_868_690_619_737_569),
    ...
    assert_eq!(
        ...
        (456, 191_056, 5_783_064_071_472_767_826),
